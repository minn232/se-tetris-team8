...
public class Board {
    ...
    public void moveLeft() {
        if (!gameOver && canMove(current, -1, 0)) {
            current.move(-1, 0);
    ...
    public boolean moveDown() {
        ...
        if (canMove(current, 0, 1)) {
            ...
            if (!full.isEmpty()) {
                ...
                pendingClearRows = full.stream().mapToInt(Integer::intValue).toArray(); // ★ 추가
                waitingLineClearAnimation = true; // UI에서 번쩍 표시 후 clearRows() 호출
            } else {
                spawnNewTetromino();
            }
            return false;
...
// 교체
    private int clearFullLines() {
        ...
        int[] toClear = rows.stream().mapToInt(Integer::intValue).toArray();
        clearRows(toClear);
...
// 교체
    ...
    public com.team.tetris.items.ItemManager getItemManager() {
        return itemManager;
    }
