...
public class BattleGamePanel extends JPanel {
    ...
    public BattleGamePanel(Difficulty difficulty, boolean isItemMode) {
        ...
        // 플레이어2 보드 (오른쪽)
        player2Board = new Board(difficulty, isItemMode, new BattleMode() {
            @Override
            public void sendLinesToOpponent(int lines) {
    ...
    private void checkFlashAnimations() {
        ...
        // 플레이어1 애니메이션 체크
        if (player1FlashingRows == null) { // 현재 플래시 중이 아닐 때만 새로운 줄 삭제 받기
            if (player1Board.hasPendingClearRows()) {
                int[] rows1 = player1Board.pollClearingRows();
                if (rows1 != null && rows1.length > 0) {
                    player1FlashingRows = rows1;
                    player1FlashUntil = now + FLASH_MS;
                    repaint(); // 플래시 시작 시 즉시 화면 갱신
                }
            }
        } else if (now >= player1FlashUntil) {
            // 플래시 애니메이션 종료 - 줄 삭제
            player1Board.clearRows(player1FlashingRows);
            player1FlashingRows = null;
            repaint(); // 줄 삭제 후 즉시 화면 갱신
        }
        
        // 플레이어2 애니메이션 체크
        if (player2FlashingRows == null) { // 현재 플래시 중이 아닐 때만 새로운 줄 삭제 받기
            if (player2Board.hasPendingClearRows()) {
                int[] rows2 = player2Board.pollClearingRows();
                if (rows2 != null && rows2.length > 0) {
                    player2FlashingRows = rows2;
                    player2FlashUntil = now + FLASH_MS;
                    repaint(); // 플래시 시작 시 즉시 화면 갱신
                }
            }
        } else if (now >= player2FlashUntil) {
            // 플래시 애니메이션 종료 - 줄 삭제
